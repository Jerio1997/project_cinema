...
//        main.test_trade_precreate();
    ...
    // 测试当面付2.0查询订单
    public AlipayF2FQueryResult test_trade_query(Integer orderId) {
        // (必填) 商户订单号，通过此商户订单号查询当面付的交易状态
        //orderService get outTradeNo
    ...
    // 测试当面付2.0生成支付二维码
    public AlipayF2FPrecreateResult test_trade_precreate(MtimeOrder mtimeOrder,MtimeCinema mtimeCinema) {
        Integer orderId = mtimeOrder.getUuid();
        ...
        String subject = "meeting"+mtimeCinema.getCinemaName();

        // (必填) 订单总金额，单位为元，不能超过1亿元
        // 如果同时传入了【打折金额】,【不可打折金额】,【订单总金额】三者,则必须满足如下条件:【订单总金额】=【打折金额】+【不可打折金额】
        String totalAmount = String.valueOf(mtimeOrder.getOrderPrice());
        ...
        int amount = mtimeOrder.getSeatsIds().split(",").length;
        String body = "购买商品"+ amount +"件共"+totalAmount+"元";
        ...
        List<GoodsDetail> goodsDetailList = new ArrayList<GoodsDetail>();
        // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        GoodsDetail goods1 =  GoodsDetail.newInstance("goods_id001", "电影",Math.round(mtimeOrder.getFilmPrice()) ,amount);
        // 创建好一个商品后添加至商品明细列表
        goodsDetailList.add(goods1);
